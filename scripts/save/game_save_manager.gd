		...
		match event.keycode:
			KEY_F5:
				if save_current_game(SaveConstants.QUICKSAVE_SLOT):
					print("[GameSaveManager] Quicksave guardado")
				else:
					print("[GameSaveManager] No se pudo guardar (¿estás en partida?)")
				get_viewport().set_input_as_handled()
			KEY_F9:
				if load_game(SaveConstants.QUICKSAVE_SLOT):
					print("[GameSaveManager] Quickload solicitado")
				else:
					print("[GameSaveManager] No hay quicksave disponible")
				get_viewport().set_input_as_handled()
...
func save_current_game(slot_name:String) -> bool:
	...
	if snapshot.is_empty():
		print("[GameSaveManager] No hay partida activa para guardar")
		return false
...
func _load_from_path(path:String) -> bool:
	...
	if file == null:
		...
		print("[GameSaveManager] %s" % err_msg)
	...
	if not (parsed is Dictionary):
		...
		print("[GameSaveManager] %s" % parse_msg)
	...
	if version != SaveConstants.SAVE_FORMAT_VERSION:
		...
		print("[GameSaveManager] %s" % ver_msg)
