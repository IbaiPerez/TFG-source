...
func change_tile_controller(tile: Tile, new_controller: Empire) -> void:
	if tile.controller:
	...
	if tile.location.type == Tile.location_type.Uncolonized:
		change_location_type(tile,VILLAGE)
...
func change_location_type(tile: Tile, new_location_type:LocationType):
	if new_location_type:
		tile.set_location_type(new_location_type)
	Events.tile_location_type_changed.emit(tile)

func _on_change_tile_controller(tile:Tile,empire:Empire):
	change_tile_controller(tile,empire)
