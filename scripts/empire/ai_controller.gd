...
func _run_turn() -> void:
	...
	print("[IA] === TURNO DE %s (turno %d) ===" % [empire_name, stats.turn_number + 1])
	...
	print("[IA] Oro: %d (+%d/turno) | Comida: %d | Tiles: %d" % [
	...
	print("[IA] %s robó %d cartas" % [empire_name, _drawn_cards.size()])
	...
	while iterations < max_iterations and not ctx.drawn_cards.is_empty():
		...
		if chosen == null or chosen.is_pass:
			print("[IA] %s decide pasar (iter %d)" % [empire_name, iterations])
			break

		print("[IA] %s juega %s" % [empire_name, chosen.describe()])
	...
	if not ctx.drawn_cards.is_empty():
		print("[IA] %s descarta %d cartas no jugadas" % [empire_name, ctx.drawn_cards.size()])
	ctx.drawn_cards = []
	_drawn_cards = []
...
func _evaluate_and_resolve_event(empire_name: String) -> void:
	...
	if event == null:
		return
	print("[IA] %s recibe evento: %s" % [empire_name, event.title])
