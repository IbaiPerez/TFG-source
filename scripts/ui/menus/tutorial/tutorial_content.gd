...
##
## Los textos NO son datos estáticos: más de la mitad de las entradas se
## componen leyendo los .tres reales de edificios, tropas y cartas tácticas
## (costes, producción, ATK/DEF, modificadores de bioma). Por eso el contenido
## vive en código y no en un recurso: así el manual no puede desincronizarse
## del balance del juego — si se toca un coste en su .tres, el tutorial lo
## refleja solo.
##
## Es RefCounted (no estático) porque los textos usan `tr()`, que es un método
## de Object.
...
func _entry_troops() -> TutorialEntry:
	var troops := [_T_MILITIA, _T_RANGED, _T_PIKEMEN, _T_CAVALRY, _T_HEAVY]
...
func _entry_tactic_cards() -> TutorialEntry:
