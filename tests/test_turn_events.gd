...
func _make_stats(p_gold: int = 100, p_food: int = 10) -> Stats:
	# event_chance 1.0: siempre dispara, para no depender del azar en los tests.
...
func test_weights_returns_the_category_weight():
	var w := _make_weights()
	w.flavour_fallback = 4.5
	assert_almost_eq(w.get_weight(EventCategory.Type.FLAVOUR), 4.5, 0.0001)
